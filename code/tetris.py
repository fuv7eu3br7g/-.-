...
def runTetris():
    ...
    while True:
        ...
        for event in pg.event.get():
            if event.type == KEYUP:
                ...
                #поворот вправо (E)
                    ...
                    fallingFig['rotation'] = (old_rotation + 1) % len(figures[fallingFig['shape']])
                    if not checkPos(cup, fallingFig):
                        fallingFig['rotation'] = old_rotation
                ...
                #поворот влево (Q)
                    ...
                    if checkPos(cup, fallingFig, adjY=1):
                        fallingFig['y'] += 1
                        fallingFig['pixel_y'] = fallingFig['y'] * block
                    last_move_down = time.time()
        ...
        if time.time() - last_fall > fall_speed:  #свободное падение фигуры
            target_pixel_y = fallingFig['y'] * block

            fallingFig['pixel_y'] += 6  #скорость анимации

            if fallingFig['pixel_y'] >= target_pixel_y + block:
                #логический шаг вниз
        ...
        #рисует окно игры со всеми надписями
...
def checkPos(cup, fig, adjX=0, adjY=0):
    #проверка нахождения фигуры в границах игрового поля, столкновений с другимим фигурами
    for x in range(fig_w):
        for y in range(fig_h):
...
def drawFig(fig, pixelx=None, pixely=None):
    ...
    if pixelx == None and pixely == None:
        pixelx = side_margin + fig['x'] * block
        pixely = top_margin + fig['pixel_y']
...
def drawnextFig(fig):  #следующая фигура
